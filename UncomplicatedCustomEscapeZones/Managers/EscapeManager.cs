...
{
    public static KeyValuePair<bool, object?>? ParseEscapeRole(Dictionary<string, List<Dictionary<string, string>>> roleAfterEscape, Player player)
    {
        ...
        {
            LogManager.Debug($"Player {player.PlayerId} is faking the team {fakeTeam} (real one: {playerTeam}), using the faked one.");
            playerTeam = fakeTeam;
        ...
        {
            LogManager.Warn($"Unable to determine player's role or team for escape evaluation (PlayerId={player.PlayerId}). Allowing natural escape.");
            return new KeyValuePair<bool, object?>(false, null);
        }

        List<Dictionary<string, string>>? entries = ResolveEntries(roleAfterEscape, $"InternalTeam {playerTeamKey}", $"IT {playerTeamKey}", $"InternalFaction {playerFactionKey}", $"IF {playerFactionKey}", $"InternalRole {playerRoleKey}", $"IR {playerRoleKey}", "all");
        ...
        {
            LogManager.Debug($"Player {player.PlayerId} has the custom keys {string.Join(", ", customKeys)}, checking for a specific escape config...");
        ...
        KeyValuePair<bool, object?>? defaultCuffedValue = null;
        bool hasDefaultCuffedValue = false;
        ...
        foreach (Dictionary<string, string> dict in entries)
        {
            ...
            foreach (KeyValuePair<string, string> kvp in dict)
            {
                ...
                {
                    defaultValue = data;
                    LogManager.Debug($"Set default escape outcome for role '{playerRoleKey}' to: {(data is null ? "Deny" : data.Value.Key ? $"CustomRole {data.Value.Value}" : $"InternalRole {data.Value.Value}")}");
                }
                else
                {
                    List<string> elements = kvp.Key.Split(' ').ToList();

                    bool isAll = elements.Count is 3 && elements[2].ToLowerInvariant() is "all";

                    if ((elements.Count != 4 && !isAll) || elements[0] is not "cuffed" || elements[1] is not "by")
                    {
                    ...
                    switch (elements[2].ToLowerInvariant())
                    {
                        ...
                        case "customteam" or "ct":
                            if (!string.IsNullOrWhiteSpace(elements[3]))
                                ...
                                LogManager.Warn($"Failed to parse custom team '{elements[3]}' for escape condition '{kvp.Key}'.");
                            break;
                        case "all":
                            defaultCuffedValue = data;
                            hasDefaultCuffedValue = true;
                            break;
                        default:
                        {
        ...
        if (player is { IsDisarmed: true, DisarmedBy: not null })
        {
            if (UCR.TryGetSummonedCustomRole(player.DisarmedBy, out object summoned))
            {
                ...
                if (customRoleId is not null && asCuffedByCustomRole.TryGetValue(customRoleId.Value, out KeyValuePair<bool, object?>? escapeRole))
                {
            ...
            foreach (string customTeam in GetCustomTeams(player.DisarmedBy, summoned))
                ...
                {
                    LogManager.Debug($"Player {player.PlayerId} disarmed by the custom team '{customTeam}', applying mapped escape outcome.");
                    return customTeamValue;
                }

            if (asCuffedByInternalRole.TryGetValue(player.DisarmedBy.Role, out KeyValuePair<bool, object?>? roleValue))
                return roleValue;
            ...
            {
                LogManager.Debug($"Player {player.DisarmedBy.PlayerId} is faking the team {disarmerFakeTeam} (real one: {disarmerTeam}), using the faked one.");
                disarmerTeam = disarmerFakeTeam;
                disarmerFaction = disarmerFakeTeam.GetFaction();
            }

            if (asCuffedByInternalTeam.TryGetValue(disarmerTeam, out KeyValuePair<bool, object?>? teamValue))
                return teamValue;

            if (asCuffedByInternalFaction.TryGetValue(disarmerFaction, out KeyValuePair<bool, object?>? factionValue))
                return factionValue;

            if (hasDefaultCuffedValue)
            {
        ...
        LogManager.Debug($"Returing default type for escaping evaluation of player {player.PlayerId} who's cuffed by team: {player.DisarmedBy?.Team} faction: {player.DisarmedBy?.Faction} role: {player.DisarmedBy?.Role}");
        return defaultValue;

        List<Dictionary<string, string>>? ResolveEntries(Dictionary<string, List<Dictionary<string, string>>> source, params string[] keys)
        {
            foreach (string key in keys)
            {
                ...
                string? ciMatch = source.Keys.FirstOrDefault(k => string.Equals(k, key, StringComparison.OrdinalIgnoreCase));
    ...
    private static KeyValuePair<bool, object?>? ParseEscapeString(string escape)
    {
        ...
        switch (elements[0])
        {
            case "CustomRole" or "CR" when int.TryParse(elements[1], out int customRoleId):
