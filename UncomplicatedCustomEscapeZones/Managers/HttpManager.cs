...
internal class HttpManager
{
    ...
    public HttpManager(string prefix)
    {
        if (!CheckForDependency())
            Timing.CallContinuously(20f,
                () => LogManager.Error(
                    "You don't have the dependency Newtonsoft.Json installed!\nPlease install it AS SOON AS POSSIBLE!\nIf you need support join our Discord server: https://discord.gg/5StRGu8EJV"));
    ...
    /// <summary>
    ///     Gets the role of the given player (as steamid@64) inside UCEZ
    /// </summary>
    ...
    private bool CheckForDependency()
    {
        return PluginLoader.Dependencies.Any(assembly => assembly.GetName().Name == "Newtonsoft.Json");
    }
    ...
    public HttpResponseMessage HttpPutRequest(string url, string content)
    {
        try
        {
            Task<HttpResponseMessage> Response = Task.Run(() =>
                HttpClient.PutAsync(url, new StringContent(content, Encoding.UTF8, "text/plain")));
    ...
    {
        return HttpGetRequest($"{Endpoint}/owners/add?discordid={discordId}")?.StatusCode ??
        ...
        try
        {
            Dictionary<string, Dictionary<string, string>> Data =
                JsonConvert.DeserializeObject<Dictionary<string, Dictionary<string, string>>>(
                    RetriveString(HttpGetRequest("https://api.ucserver.it/credits.json")));
            ...
            foreach (KeyValuePair<string, Dictionary<string, string>> kvp in Data.Where(kvp =>
                         kvp.Value is not null && kvp.Value.ContainsKey("role") && kvp.Value.ContainsKey("color") &&
                         kvp.Value.ContainsKey("override")))
            {
                Credits.Add(kvp.Key,
                    new Triplet<string, string, bool>(kvp.Value["role"], kvp.Value["color"],
                        bool.Parse(kvp.Value["override"])));
    ...
    public void ApplyCreditTag(Player player)
    {
        ...
        if (player.ReferenceHub.serverRoles.Network_myText is not null &&
            player.ReferenceHub.serverRoles.Network_myText != string.Empty)
        {
            if (Credits.Any(k =>
    ...
    internal async Task<Tuple<HttpStatusCode, string?>> VersionInfo()
    {
        HttpResponseMessage message =
            await HttpClient.GetAsync(
                $"{Endpoint.Replace("/v2", "")}/vinfo/info?v={Plugin.Instance.Version.ToString(4)}&labapi=true");

        if (message.StatusCode != HttpStatusCode.OK)
            return new Tuple<HttpStatusCode, string?>(message.StatusCode, null);

        return new Tuple<HttpStatusCode, string?>(message.StatusCode, await message.Content.ReadAsStringAsync());
    }
}